...
func createGiftCode(c *gin.Context) {
	...
	case 1: // 1 - 指定用户一次性消耗
		...
		if gift.ReceivingUser == "" {
			c.JSON(402, gin.H{MESSAGE: "请指定领取用户"})
			return
		}
	case 2: // 2 - 不指定用户限制兑换次数
		if gift.AvailableTimes <= 0 {
			c.JSON(403, gin.H{MESSAGE: "请指定可兑换次数"})
			return
		}
	case 3: // 3 - 不限用户不限次数兑换 无用户限制 无兑换次数限制这里不做处理
	default: // 非法填写的礼品码类型
		c.JSON(404, gin.H{MESSAGE: "礼品码类型不合法"})
		return
	}

	// 检查礼品码描述
	if gift.Description == "" {
		c.JSON(405, gin.H{MESSAGE: "请输入礼品码描述信息"})
		return
	}

	// 检查礼品码有效期是否为空
	if gift.ValidPeriod == "" {
		c.JSON(406, gin.H{MESSAGE: "请输入有效期"})
		return
	}
	...
	if err != nil {
		c.JSON(407, gin.H{MESSAGE: "输入的礼品码有效期格式不正确"})
		return
	}
	exp := vp.Sub(time.Now())
	if exp <= 0 {
		c.JSON(408, gin.H{MESSAGE: "输入的礼品码有效期小于当前时间"})
		return
	}
	gift.Expiration = exp
	...
	if gift.GiftPackages == nil || len(gift.GiftPackages) == 0 {
		c.JSON(409, gin.H{MESSAGE: "请输入礼包内容"})
		return
	}
	var n = 5
	for {
...
func queryGiftCode(c *gin.Context) {
	...
	if giftCode == nil {
		c.JSON(401, gin.H{MESSAGE: "礼品码输入错误或礼品码已过期"})
		return
	}
...
func verifyGiftCode(c *gin.Context) {
	...
	// 检查礼品码
	if req.Code == "" {
		c.JSON(402, gin.H{MESSAGE: "请输入礼品码"})
		return
	}

	// 检查领取用户
	if req.User == "" {
		c.JSON(403, gin.H{MESSAGE: "请输入领取用户"})
		return
	}
